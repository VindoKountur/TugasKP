		...
		<form action="buat.php" method="GET">
			<?php 
				if (isset($_GET['update'])) {
					$id = $_GET['id'];
					$query = mysqli_query($db,"SELECT * FROM driver WHERE id_driver = '$id'");
			...
			?>
			<div class="head">
				<label>Tambah Daftar Sopir</label>
			</div>
			<div class="row m5">
				<div class="col-25"><label>Nama<label></div>
				<div class="col-75">
					: <input type="text" name="nama" required <?php if(isset($_GET['update'])){ echo "value='$data[nama_driver]'"; } ?>>
				</div>
			</div>
			<div class="row m5">
				<div class="col-25"><label>No Telepon<label></div>
				<div class="col-75">:
					<input type="text" name="notelepon" required id="tel" <?php if(isset($_GET['update'])){ echo "value='$data[no_telp]'"; } ?>>
				</div>
			</div>
			<div class="row">
